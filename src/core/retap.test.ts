...
interface ResolveRefresh {
  (): void
}
describe('createRetapRefreshCore registry and runner', function () {
  ...
  it('同一 key 执行中再次触发返回 false', async function () {
    const core = createRetapRefreshCore<'profile'>()
    let resolveRefresh: ResolveRefresh | undefined
...
describe('createRetapRefreshCore animation state', function () {
  it('开始和结束动画时能同步当前 key', function () {
    const core = createRetapRefreshCore<'recommend' | 'orders' | 'profile'>()
    const states: Array<'recommend' | 'orders' | 'profile' | ''> = []
    const unsubscribe = core.subscribeRefreshAnimation(function (key) {
      states.push(key)
    })
  ...
  it('不同 key 可以切换动画归属，旧 key 不会误停新动画', function () {
    const core = createRetapRefreshCore<'recommend' | 'orders'>()
    expect(core.startRefreshAnimation('recommend')).toBe(true)
  ...
  it('unsubscribe 后不再收到状态更新', function () {
    const core = createRetapRefreshCore<'recommend'>()
