...
describe('createRetapRefreshContext', function () {
  it('enabled=true 挂载后注册，卸载后注销', async function () {
  ...
  it('enabled 从 false 到 true 再到 false 时同步注册状态', async function () {
  ...
  it('卸载后停止 watch，后续改 enabled 不再影响 context', async function () {
  ...
  it('useRetapRefreshAnimation 卸载后会清掉当前动画 key', async function () {
    const context = createRetapRefreshContext<'recommend'>()
    const mounted = mountSetup(function () {
