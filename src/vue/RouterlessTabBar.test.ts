...
describe('RouterlessTabBar', function () {
  it('渲染 items 文案', function () {
  ...
  it('点击非当前 tab emits change', async function () {
    const wrapper = mount(RouterlessTabBar, {
      props: {
        active: 'recommend',
        items,
  ...
  it('点击当前 tab emits retap', async function () {
    const wrapper = mount(RouterlessTabBar, {
      props: {
        active: 'recommend',
        items,
  ...
  it('active item 使用 selectedIconPath', function () {
    const wrapper = mount(RouterlessTabBar, {
      props: {
        active: 'recommend',
        items,
  ...
  it('缺失 selectedIconPath 时回退到 iconPath', function () {
    const wrapper = mount(RouterlessTabBar, {
      props: {
        active: 'recommend',
        items: [
  ...
  it('缺失 icon 时不渲染 image，但 text 仍显示', function () {
    const wrapper = mount(RouterlessTabBar, {
      props: {
        active: 'profile',
        items: [
  ...
  it('refreshing item 带 refreshing class 和 active class', function () {
    const wrapper = mount(RouterlessTabBar, {
      props: {
        active: 'recommend',
        items,
  ...
  it('refreshIcon 存在时显示 refresh icon 并隐藏默认 text/icon', function () {
    const wrapper = mount(RouterlessTabBar, {
      props: {
        active: 'recommend',
        items,
  ...
  it('refreshIcon 缺失时回退普通内容', function () {
    const wrapper = mount(RouterlessTabBar, {
      props: {
        active: 'recommend',
        items,
  ...
  it('item 插槽能拿到完整 slot props，并替换默认内容', function () {
  ...
  it('使用 item 插槽后点击非当前 tab 仍 emits change', async function () {
    const wrapper = mount(RouterlessTabBar, {
      props: {
        active: 'recommend',
        items,
      ...
      slots: {
        item: function ({ item }) {
          return h('div', { class: 'slot-content' }, item.text)
        },
      },
    })
    await wrapper.findAll('.routerless-tabbar-item')[1]?.trigger('click')
  ...
  it('使用 item 插槽后点击当前 tab 仍 emits retap', async function () {
    const wrapper = mount(RouterlessTabBar, {
      props: {
        active: 'recommend',
        items,
