...
| `resolveTabPageModuleKey`   | 纯函数   | 把 `/pages/...` 转成页面模块 key        |
...
把业务 Tab 的页面路径转换为 main 页面中 `import.meta.glob('../*/index.vue')` 可用的模块 key。
...
- subpackages、自定义页面根目录、非同级 main 页面等复杂结构，建议业务自行实现 resolver 或直接在 Tab 配置中声明模块 key。
