...
- [README](../README.md)：对外版本承诺、示例分层和文档总入口。
- [多端兼容性说明](./compatibility.md)：当前稳定支持范围与 experimental 边界。
- [端侧运行时验证矩阵](./runtime-validation.md)：发布前各验证层级的实际含义。
- [API 文档](./api.md)：发布前需要保持稳定的导出面与类型契约。
