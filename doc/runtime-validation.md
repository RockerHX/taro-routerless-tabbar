...
| 小程序手动端侧验证   | 开发者工具 / 真机                  | 验证平台运行时样式、点击和页面栈。    |
...
| -------------------- | -------- | ------ | ---------- | ------------ | ------------ | ---------- |
| `tab` query 初始化   | 已覆盖   | 已覆盖 | 已覆盖     | 已构建       | 已构建       | 待记录     |
| Tab 切换 active 状态 | 已覆盖   | 已覆盖 | 已覆盖     | 已构建       | 已构建       | 待记录     |
| 已访问 pane 保活     | 已覆盖   | 已覆盖 | 已覆盖     | 已构建       | 已构建       | 待记录     |
| retap 异步刷新       | 已覆盖   | 已覆盖 | 已覆盖     | 已构建       | 已构建       | 待记录     |
| redirect query 合并  | 已覆盖   | 已覆盖 | 已覆盖     | 已构建       | 已构建       | 待记录     |
| 返回链路             | 未覆盖   | 已构建 | 已覆盖     | 已构建       | 已构建       | 待记录     |
| 长列表底部 padding   | 未覆盖   | 已构建 | 已覆盖     | 已构建       | 已构建       | 待记录     |
| 样式边界 class/变量  | 已覆盖   | 已覆盖 | 已覆盖     | 已构建       | 已构建       | 待记录     |
...
| 待记录     | 4.2.0 | 3.x | WeChat 开发者工具对应插件 | 开发者工具 / 真机 | 待手动验证                                              |

## 已知限制

- WeChat 小程序当前自动化只覆盖 build smoke，不代表开发者工具或真机运行时已自动验证。
